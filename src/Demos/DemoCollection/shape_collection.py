...
from ..Scenes.shape_scene import Shape_Scene
...
def generate_shape_dataset(
    ...
    scene = Shape_Scene(boundary_restiction)
    ...
    n_shapes = len(scene.get_supported_shapes())
    print(f"Making {n_episodes} demos for {n_shapes} shapes", end="")
    n_episodes = n_episodes * n_shapes
    print(f" for a Total of {n_episodes} demos")
        ...
        while r < n_runs:
            ...
            if task_completed:
                ...
                if taskFail_counter == 5:
                    ...
                    r = n_runs
                    ep -= 1
                    print(f"\033[31mObject position [{gen.target.get_position()[:2]}] is likely invalid, generating new cube\033[38;5;231m", end="")
                else:
