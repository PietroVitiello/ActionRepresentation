...
class Quadratic():
    ...
    def __init__(self, tip, target, max_deviation=0.05) -> None:
        ...
        self.linear_mid = Dummy.create(0.05)
    ...
    def find_quadratic(self):
        intersection = np.linalg.norm(self.distance)/2
        a = self.apex/(intersection**2)
        c = self.apex
        return np.array([a, c])

    def get_archLen(self):
        return np.linalg.norm(self.distance)

    def getGradient(self, ik_tip: Object):
        rel_pos = ik_tip.get_position(relative_to=self.linear_mid)
        print(f"\nrelative position: {rel_pos}")
        print(f"distance/2: {np.linalg.norm(self.distance)/2}")
    ...
    def find_middlePoint(self):
        ...
        self.apex = np.random.uniform(self.max_deviation)
        self.ortho = self.get_axis()
        self.eq = self.find_quadratic()

        # n = Dummy.create(0.1)
        # n.set_orientation(self.linear_mid.get_orientation())
        # n.set_position(self.linear_mid.get_position()+0.1*self.ortho)
    ...
    def get_tangentVelocity(self, ik_tip: Object, v: np.ndarray):
        ...
        print(F"transformed v: {np.linalg.inv(R) @ v}")
        print(F"rotated v: {rotation @ np.linalg.inv(R) @ v}")
